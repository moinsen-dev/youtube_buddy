...
**`CloudEngine` (drittes Backend):** Implementiert dasselbe `LLMEngine`-Interface (`id: 'cloud'`) gegen **Firebase AI (Gemini)** via Cloud Function (EU) im selben GCP-Projekt — kein Dritt-Provider, keine Key-Verwaltung in der App. Prompt-Templates, zod-Schemata und Golden-Set-Gate sind identisch zum On-Device-Pfad; strukturierte Outputs laufen über `responseSchema`/`response_mime_type: application/json` (Gemini-nativ, analog zum GBNF-Modus bei llama.cpp). Ergebnisse tragen in `analyses.model` die Cloud-Kennung (z. B. `gemini-flash`). Aktivierung pro Nutzeraktion (Opt-in), UI-Badge „Cloud (Opt-in)" als Invers des On-Device-Badges. Fallback: bei Fehler/Offline → On-Device-Engine.
