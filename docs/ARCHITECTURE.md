...
**Sync-Engine (`core/sync`, Phase 10.5):** Supabase (EU) als Transport. Ablauf: lokale SQLite-Entitäten → Client-seitige Verschlüsselung (Key aus Passphrase/Device-Key + Recovery-Code) → Upsert pro Tabelle mit `updated_at`-Last-Write-Wins → Pull-Delta beim Start. Nicht synchronisiert werden: Modelldateien, Transkript-Roh-Chunks (bei Bedarf neu abrufbar), Quota-/Cache-Tabellen. Auth via Supabase-Auth (Google-Provider, gleiche OAuth-Identität wie YouTube-Login). Entitlements via RevenueCat, lokal gecacht (Offline-Weiternutzung von Pro-Features bis Ablauf).
