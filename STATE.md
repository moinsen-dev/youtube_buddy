...
**Befunde (für später):** drizzle/expo-sqlite ist durchgehend sync-basiert (`executeSync`) — auf Web geht das über den SAB-Pfad und hängt in Sequenzen/Transaktionen; rohe async Calls sind stabil. Web-Screens, die viele drizzle-Schreibzugriffe machen (Sync-Push, Settings), brauchen denselben Raw-Pfad, falls sie auf Web laufen sollen. **COOP-Falle (2026-07-21, User-Report):** `Cross-Origin-Opener-Policy: same-origin` kappt das Google-OAuth-Popup (Token kommt nie zurück) — gelöst via `same-origin-allow-popups`; die Web-DB braucht `crossOriginIsolated` gar nicht (async wa-sqlite + Raw-Executor kommen ohne SharedArrayBuffer aus).
